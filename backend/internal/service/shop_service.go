...
func (s *ShopService) ListPagedFiltered(page, limit int, filter string, query string) ([]model.Shop, int64, error) {
	now := time.Now()
	return s.shops.ListPagedFiltered(page, limit, filter, query, now)
}
