...
func (h *ShopHandler) CreateShop(c *gin.Context) {
	...
	price := 0
	if req.PricePerCycle != nil {
		price = int(*req.PricePerCycle)
	}
	cycle := 0
...
func (h *ShopHandler) ListShops(c *gin.Context) {
	...
	items, total, err := h.svc.ListPagedFiltered(page, limit, filter, q)
	if err != nil {
...
func (h *ShopHandler) UpdateShop(c *gin.Context) {
	...
	if req.PricePerCycle != nil || req.CycleMonths != nil {
		...
		var cm *int
		if req.PricePerCycle != nil {
			v := int(*req.PricePerCycle)
			pp = &v
		}
		if req.CycleMonths != nil {
			v := int(*req.CycleMonths)
			cm = &v
		}
...
func (h *ShopHandler) ListAllAPILogs(c *gin.Context) {
	...
	var status *string
	if v := c.Query("domain_param"); v != "" {
		domainParam = &[]string{v}[0]
	}
	if v := c.Query("uuid_param"); v != "" {
		uuidParam = &[]string{v}[0]
	}
